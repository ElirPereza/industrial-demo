...
export default function ContratistasPage() {
	...
	const calificacionProm =
		contratistas.length > 0
			? Math.round(
					(contratistas.reduce((sum, c) => sum + c.calificacion, 0) /
						contratistas.length) *
						10,
				) / 10
			: 0
		...
		<SidebarInset>
			...
			<div className="flex flex-1 flex-col gap-6 p-4 pt-0">
				<div className="flex items-center justify-between">
					<div>
						<h1 className="text-2xl font-semibold">Gestión de Contratistas</h1>
				...
				<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
					<Card>
						<CardContent className="flex items-center gap-4 p-4">
							...
							<div>
								<p className="text-2xl font-semibold">{contratistas.length}</p>
								<p className="text-sm text-muted-foreground">
									Total Contratistas
								</p>
							</div>
										...
										<span
											className={cn(
												...
												contratista.contrato_vigente &&
													new Date(contratista.contrato_vigente) < new Date()
													? "text-red-600"
													: "text-green-600",
											)}
										>
